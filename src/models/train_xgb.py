...
def get_xy(add_features: bool, drop: bool):
    """Prepara X (com/sem engenharia) e y codificado."""
    df = prepare(load_raw(), add_features=add_features, drop=drop)
...
def run(add_features: bool, drop: bool, label: str, verbose: bool = True,
        tune: bool = False):
    X, y, le = get_xy(add_features, drop)
...
def main() -> None:
    ...
    # Comparação: dados originais vs. seleção+engenharia de atributos
    print("Comparando o efeito da engenharia/seleção de atributos...")
    _, _, _, base = run(add_features=False, drop=False,
